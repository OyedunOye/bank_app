          ...
          {footerLinks.map((footerLink)=>(
            <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              ...
              <ul className="list-none mt-4">{footerLink.links.map((link,index)=>(
                <li key={link.name} className={`font-poppin font-normal text-[16px] text-dimWhite leading-[24px]
                hover:text-secondary cursor-pointer ${index!== footerLink.links.length-1 ? 'mb-4':'mb-0'}`}>{link.name}</li>
              ))}
              </ul>
            </div>
          ))}
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]">
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          {socialMedia.map((social, index)=>(
            ...
            cursor-pointer ${index!== socialMedia.length-1 ? 'mr-4':'mr-0'}`} />
          ))}
        </div>
      </div>
    </section> 
